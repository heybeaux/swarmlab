...
### RT-26 — Known effect failure needs bound negative evidence, not permanent indeterminacy (exp-35)

- **Finding:** RT-23 correctly preserved unknown started effects as indeterminate, but RT-25 added only verified success. Baseline `efrb-mtwl39ot` therefore missed both known-failure cases (`2/10`), achieved failure accuracy `2/10`, exposed no failure API, and could not replay a negative outcome idempotently; the receipt-bound fixture stayed green.
- **Change:** Aegis `8240fa5` adds `failExecutionEffect()`, exact permit/approval/operation binding, strict digest and failure-code validation, verified negative evidence, a durable `failed` state, conflict-safe exact replay, and committed-success monotonicity. The unchanged roster reran as `efrb-mtwl5l6q`: all unsafe/missed rates `0`; accuracy/API/coverage/idempotent/monotonicity metrics `1`.
- **Boundary:** the host must independently prove non-commitment. Unknown post-start transport outcomes remain indeterminate, and a retry needs new authority; negative receipts are evidence, not a shortcut around exactly-once limits.

### RT-27 — Lost terminal acknowledgements need exact receipt readback (exp-36)

- **Finding:** RT-25/26 made success and failure receipt writes atomic and monotonic, but both public boundaries collapsed every store exception into `indeterminate`. Baseline `tra-mty0gffk` therefore orphaned `2/10` committed terminal receipts, misclassified `4/10` scenarios, and achieved reconciliation accuracy `0.600` while the exact-readback fixture stayed green.
- **Change:** Aegis `2238094` now reads the retained effect after a terminal-write exception and accepts only an exact permit/approval/operation/digest/verification (plus failure-code) match. A different terminal winner is a receipt conflict; absent, non-terminal, or unavailable readback remains honestly indeterminate.
- **Same-experiment proof:** unchanged `tra-mty0hgii` moved orphan, misclassification, conflicting-acceptance, and pre-commit false-terminal rates to `0`; accuracy/API/coverage/idempotent/monotonic/unavailable-read safety all reached `1`.
- **Boundary:** the host still owns atomic first-terminal-write, exact receipt retention, and linearizable readback. Aegis cannot infer truth when that read is unavailable, and this result does not make the external effect itself transactional.

### RT-28 — Terminal state and receipt are one integrity envelope (exp-37)

- **Finding:** RT-27 recovered exact terminal writes, but generic `resolveExecutionEffect()` still trusted `committed`/`failed` state without validating its retained receipt and could return a retryable clean-state answer while a terminal fragment was present. Baseline `tji-mtzfwl8k` produced false terminal certainty `1.000`, unsafe retry `0.500`, integrity error `0.714`, and accuracy `0.286`; the deterministic coherence fixture was green.
...
### RT-29 — Coherent replica reads still need monotonic causal truth (exp-38)

- **Finding:** RT-28 rejects internally split terminal records, but a perfectly coherent older replica remains dangerous. Baseline `mjr-mu0vdd5j` on Aegis `648cad1` produced stale retry authority `0.333`, stale classification error `1.000`, revision-failure unsafe rate `1.000`, accuracy `0.308`, and zero revision API availability. An older authorized snapshot could regain retry authority after a newer committed or failed revision, and a post-CAS stale read hid a concurrent terminal winner.
- **Change:** Aegis `c474322` adds the optional `RevisionedApprovalExecutionPermitStore` contract. Revision-capable stores expose authoritative monotonic high-water truth retained across failover/restore; Aegis accepts a visible record only when its positive safe-integer revision equals that watermark. Older or missing-under-watermark reads are `journal_stale`; malformed/ahead truth is inconsistent; unavailable watermarks fail closed. The validation covers resolution, pre-start, failed-CAS readback, and terminal acknowledgement reconciliation while legacy stores remain compatible.
- **Same-experiment proof:** unchanged `mjr-mu0vempr` moved all unsafe/error rates to `0` and accuracy/API/coverage/current-state/post-CAS metrics to `1`.
- **Boundary:** the host owns monotonic revision assignment, durable high-water/tombstone retention, and authoritative reads. Aegis cannot repair replication, but it must not convert observable causal rollback into execution authority or obsolete certainty.
...
### RT-31 — Terminal truth must survive deliberate journal compaction (exp-40)

- **Finding:** RT-28 through RT-30 make full receipt-bearing journal records trustworthy, current, and write-attested, but baseline Aegis `6f24d45` had no way to preserve terminal certainty after intentional record pruning. Run `tcp-mu3q8xw5` recovered neither compacted terminal outcome, exposed no compaction-proof API, and classified only `5/17` scenarios correctly while the strict authenticated-proof fixture was green.
- **Change:** Aegis `8c2cc74` adds an optional `CompactedApprovalExecutionPermitStore`, strict host-authenticated terminal proof binding (operation, permit, approval, outcome, exact receipt digest/failure code, and monotonic revision), an explicit `resolveCompactedExecutionEffect()` entry point, and capability detection in the ordinary resolver and begin fence. Invalid/missing/stale/future/unavailable proof truth fails closed; proof-only terminal state is never retryable.
- **Same-experiment proof:** unchanged `tcp-mu3qbgoq` reached proof recovery and resolution accuracy `1`, API availability `1`, every preservation/safety/coverage metric `1`, and both false-proof acceptance and stale retry authority `0`.
- **Boundary:** the host still owns atomic compaction, durable proof retention, proof authentication, and authoritative revision truth. The experiment proves the adapter/policy contract with deterministic records; it does not certify a production database, signing algorithm, or retention service.


### RT-32 — Authority-plane rollback requires an independent checkpoint (from exp-41)

- **Finding:** RT-29's host high-water and RT-31's compact proof are strong only while their authority plane survives. A coherent backup restore of the journal, high-water, and proof made current Aegis accept the restored revision and return retry authority for an effect that had advanced to revision 3.
- **Experiment:** Spec 47 froze seventeen deterministic scenarios across host-only control, strict transparency-anchor fixture, and real built Aegis. Baseline `ara-mu55rn5j` on `b50b46d` detected `1/5` rollback cases, restored retry authority in `1/5`, reached `7/17` exact accuracy, and had no checkpoint API. No LLM judged success.
- **Change and retest:** Aegis `f5d7d7e` adds an optional independently checkpointed store contract, strict exact checkpoint validation, lower-bound comparison (`hostRevision < checkpointRevision` is stale; a lagging checkpoint is safe), and checks at resolve, begin, and losing-CAS readback boundaries. Same frozen roster `ara-mu55z5e7` detected `5/5`, restored zero retry authority, and passed all seventeen scenarios plus every preservation/safety/coverage metric.
- **Boundary:** The host must authenticate and retain the checkpoint outside the journal restore domain. Aegis cannot detect rollback if the checkpoint itself is rolled back with every other source. The experiment proves adapter and policy behavior, not a production transparency log or signature scheme.

### RT-33 — Checkpoint authority equivocation needs multi-authority consistency (from exp-42)

- **Finding:** RT-32's single independent checkpoint still lets a host choose one valid-looking authority while hiding another independently valid authority that reports a conflicting same-operation history. Baseline `cae-mu6lbz2a` on Aegis `4a30255` detected `0/5` equivocation cases, restored retry/execute authority in `3/5`, reached `5/16` exact accuracy, and exposed no plural checkpoint API while the strict fixture stayed green.
- **Change and retest:** Aegis `77883f6` adds an optional plural checkpoint-authority contract, exact authority identity and `historyDigest` validation, duplicate-authority rejection, same-revision conflict detection, and an explicit `resolveMultiAuthorityAnchoredExecutionEffect()` boundary. Same frozen roster `cae-mu6lbz32` detected `5/5`, restored zero retry/execute authority, reached `16/16` exact accuracy, and passed every preservation, safety, post-CAS, ask, and consume metric.
- **Boundary:** Aegis validates exposed authority consistency; it cannot discover witnesses the host does not provide, nor can it defeat colluding authorities that all return the same rolled-back history. The production transparency quorum, signing, witness-retention, and completeness policy remain host-owned.

### RT-34 — Checkpoint witness-set completeness is a separate contract (from exp-43)

- **Finding:** RT-33 validated consistency among visible authorities, but not completeness. Baseline `cwo-mu80pubb` on Aegis `8eb1c32` detected `0/5` omitted-witness cases, restored retry/execute authority in `3/5`, reached `4/14` exact accuracy, and exposed no witness-set API while the strict fixture stayed green.
- **Change and retest:** Aegis `a62c52e` adds an optional `readEffectRevisionWitnessSet()` contract, required authority id validation, minimum-quorum checks, missing/invalid/unavailable fail-closed behavior, and explicit `resolveWitnessSetAnchoredExecutionEffect()`. Same frozen roster `cwo-mu80u7bx` detected `5/5`, restored zero retry/execute authority, reached `14/14` exact accuracy, and passed every preservation, safety, post-CAS, ask, and consume metric. RT-34 evidence gate landed in `c24b37c`.
- **Boundary:** Aegis validates an exposed authenticated witness-set policy; it cannot discover hidden witnesses or prove the production signing/retention service is complete. Witness discovery, authentication, and retention remain host-owned.

### RT-35 — Checkpoint witness roster freshness needs an epoch/digest contract (from exp-44)
...
### RT-38 — Durable strict-roster retirement needs terminal-bound tombstones (exp-47)

Exp-46 kept exact strict-roster markers safe across restart but left lifecycle open. The frozen exp-47
roster showed why deletion is not retirement: naive delete-on-terminal restored authority in 58.3%
of failure scenarios and reached only 0.25 exact accuracy. Real Aegis `942e7be` exposed no retirement
surface (`dsrt-muglbjj3`: failure detection 0, authority restoration 1, accuracy/API 0).
